...
stan_glmer <- function(formula, data, family = "gaussian", prefix = c("b_","v_"),
                      default_prior = "dnorm(0,10)", loc_scale_transform = TRUE,
                      file_name = NULL, run_stan = FALSE, ...) {

  if (is.null(file_name)) {
    if (!run_stan) stop("'run_stan' should be set to TRUE if 'file_name' is not specified.")
    file_name <- tempfile(pattern = "stan_code", fileext = ".stan")
  }
